...
APP_TITLE = "RheoDB — HB / Cross / Ellis + Cross Partial Yield 流变分析"
...
Y_LABELS = {
    ...
    "torque_uNm": "Raw torque M (μN·m)",
}

X_LABELS = {
    "shear_rate_1_s": "Shear rate γ̇ (1/s)",
    "shear_stress_Pa": "Shear Stress τ (Pa)",
    "angular_velocity_rad_s": "Angular velocity Ω (rad/s)",
}

PLOT_PAIR_OPTIONS = {
    "τ vs γ̇：Shear stress vs shear rate": ("shear_rate_1_s", "shear_stress_Pa"),
    "η vs γ̇：Viscosity vs shear rate": ("shear_rate_1_s", "viscosity_Pa_s"),
    "η vs τ：Viscosity vs shear stress": ("shear_stress_Pa", "viscosity_Pa_s"),
    "M vs Ω：Torque vs angular velocity（新增）": ("angular_velocity_rad_s", "torque_uNm"),
    "τ vs Ω：Stress vs angular velocity（新增）": ("angular_velocity_rad_s", "shear_stress_Pa"),
    "η vs Ω：Viscosity vs angular velocity（新增）": ("angular_velocity_rad_s", "viscosity_Pa_s"),
}
...
def _prepare_xy(
    ...
    *,
    ...
    """Prepare x/y arrays for plotting and error comparison.

    x_col can be shear_rate_1_s or shear_stress_Pa, so the multi-experiment
    comparison can plot both conventional flow curves and viscosity-vs-stress curves.
    """
    d = _get_segment_df(df0, seg)
    if x_col not in d.columns:
        return np.array([]), np.array([])
    if y_col == "torque_uNm":
        if "torque_Nm" not in d.columns:
            return np.array([]), np.array([])
        y_series = pd.to_numeric(d["torque_Nm"], errors="coerce") * 1.0e6
    else:
        if y_col not in d.columns:
            return np.array([]), np.array([])
        y_series = pd.to_numeric(d[y_col], errors="coerce")
...
def _parse_float_list(text: str) -> list[float]:
    """Parse comma / semicolon / whitespace separated numbers for custom axis ticks."""
    ...
    for part in cleaned.split(","):
        ...
        try:
            ...
            continue
    # 去重并排序，避免 matplotlib 报 tick 顺序问题
    return sorted(set(vals))


def _apply_compare_axis_style(
    ax: Any,
    *,
    x_axis_scale: str,
    y_axis_mode: str,
    custom_y_ticks_text: str,
) -> None:
    """Apply axis scale/tick choices for the multi-experiment overlay plot."""
    if x_axis_scale == "log":
        try:
            ax.set_xscale("log")
        except Exception:
    ...
    if y_axis_mode == "log：10^n":
        try:
            ...
            pass
    elif y_axis_mode == "linear：自定义刻度":
        ticks = _parse_float_list(custom_y_ticks_text)
        if ticks:
            try:
...
def _full_compare_label(row: pd.Series, ref_id: int) -> str:
    """Experiment label used in the overlay legend, without created_at time."""
    sid = int(row["id"])
    sname = str(row.get("sample_name") or "")
    geom = GEOMETRY_LABELS.get(str(row.get("geometry") or ""), str(row.get("geometry") or ""))
    label = f"{sid} | {sname} | {geom}"
    try:
        if int(row["id"]) == int(ref_id):
            label += " [reference]"
    except Exception:
        pass
    return label
...
def _overlay_segments_from_mode(mode: str) -> list[str]:
    """Return the plotted sweep segments for the overlay figure."""
    if mode == "up + down 同一张图":
        return ["up", "down"]
    if mode == "只画 up":
        return ["up"]
    if mode == "只画 down":
...
def _plot_multi_experiment_overlay(
    *,
    db_path: Path,
    ...
    """Plot up/down measured curves in one clean overlay figure; no fit/trend lines."""
    fig, ax = plt.subplots(figsize=(11.2, 6.2))
    cmap = plt.get_cmap("tab20")
    color_map = {int(eid): cmap(i % cmap.N) for i, eid in enumerate(selected_ids)}

    # 和示例图一致：同一实验同一颜色；up/down 用 marker 区分，不再画任何 fit 直线。
    marker_map = {"up": "o", "down": "s", "all": "D"}
    linestyle_map = {"up": "-", "down": "-", "all": "-"}
    plotted = 0
    segments_to_plot = _overlay_segments_from_mode(overlay_mode)

    for eid in selected_ids:
        df0 = rdb.measurements_df(db_path, int(eid))
        base_label = _full_compare_label(id_to_row[int(eid)], ref_id)
        color = color_map[int(eid)]
        ...
        for seg in segments_to_plot:
            ...
            seg_label = seg if seg in {"up", "down"} else "all"
            label = f"{base_label} — {seg_label}"
            ax.plot(
                x,
                y,
                marker=marker_map.get(seg, "o") if show_markers else None,
                markersize=4.2,
                linewidth=1.45,
                linestyle=linestyle_map.get(seg, "-"),
                alpha=0.90,
                color=color,
                label=label,
            )
            ...
            if show_end_labels:
                try:
                    end_label = str(id_to_row[int(eid)].get("sample_name") or f"exp {eid}")
                    if int(eid) == int(ref_id):
                        end_label += " [ref]"
                    end_label += f" — {seg_label}"
                    ax.annotate(
                        end_label,
                        xy=(x[-1], y[-1]),
                        xytext=(5, 0),
    ...
    _apply_compare_axis_style(
        ax,
        x_axis_scale=x_axis_scale,
        y_axis_mode=y_axis_mode,
        custom_y_ticks_text=custom_y_ticks_text,
    )
    ax.set_xlabel(X_LABELS.get(x_choice, x_choice))
    ax.set_ylabel(Y_LABELS.get(y_choice, y_choice))
    ax.set_title(f"Selected experiments overlay: {Y_LABELS.get(y_choice, y_choice)} vs {X_LABELS.get(x_choice, x_choice)}")
    ax.grid(True, which="both", alpha=0.28)

    # 默认放在图内左上角，样式接近你给的参考图；实验很多时也尽量压缩字体。
    if plotted:
        ncol = 1 if plotted <= 8 else 2
        ax.legend(
            loc="upper left",
            fontsize=7.3,
            frameon=True,
...
with TAB_IMPORT:
    ...
    st.caption("当前版本保留 Herschel–Bulkley (HB)、Cross 和 Ellis，并新增 Fig8–Fig10：Torque / Stress / Viscosity vs Angular Velocity。")
        ...
        if not sample_name.strip():
            ...
            try:
                ...
                if run_analysis:
                    ...
                    figs = [res["artifacts"].get(k) for k in ["fig1", "fig2", "fig3", "fig4", "fig5", "fig6", "fig7", "fig8", "fig9", "fig10"]]
                    cols = st.columns(2)
    ...
    if not db_path.exists():
        ...
        if exp_df.empty:
            ...
            with right:
                ...
                # Cross workflow:
                # 1) run a free-m fit first;
                # 2) optionally fix m for any segment and re-fit eta0, eta_inf and sigma_c.
                current_fit_df = rdb.fit_results_df(db_path, int(exp_id))
                raw_for_cross_settings = rdb.measurements_df(db_path, int(exp_id))
                detected_segments = list(rdb.split_up_down(raw_for_cross_settings).keys())
                cross_m_overrides: dict[str, float] = {}

                with st.expander("Cross 拟合设置：先自由拟合，再手动固定 m", expanded=False):
                    st.markdown(
                        "首次分析建议使用 **自由拟合 m**。之后可只对指定的 up / down / all "
                        "勾选固定 m；重新拟合时，η₀、η∞ 和 σc 会在该 m 下自动重新优化。"
                    )
                    st.caption("未勾选的 segment 仍然自由拟合；因此 shear-stress-control 可以继续使用 free m。")

                    current_cross = pd.DataFrame()
                    if not current_fit_df.empty and {"model", "segment"}.issubset(current_fit_df.columns):
                        current_cross = current_fit_df.loc[current_fit_df["model"] == "cross"].copy()

                    for seg_name in detected_segments:
                        existing_m = 10.0
                        existing_method = ""
                        if not current_cross.empty:
                            rows_seg = current_cross.loc[current_cross["segment"] == seg_name]
                            if not rows_seg.empty:
                                row_seg = rows_seg.iloc[-1]
                                try:
                                    m_candidate = float(row_seg.get("m"))
                                    if np.isfinite(m_candidate) and m_candidate > 0:
                                        existing_m = m_candidate
                                except Exception:
                                    pass
                                existing_method = str(row_seg.get("method") or "")

                        default_fixed = "fixed_m" in existing_method
                        c_fix, c_m = st.columns([1.05, 1.35])
                        with c_fix:
                            use_fixed = st.checkbox(
                                f"固定 {seg_name} 的 m",
                                value=default_fixed,
                                key=f"cross_fix_m_{int(exp_id)}_{seg_name}",
                            )
                            if existing_method:
                                mode_text = "fixed m" if default_fixed else "free m"
                                st.caption(f"当前结果：{mode_text}，m = {existing_m:.5g}")
                            else:
                                st.caption("尚无 Cross 拟合结果")
                        with c_m:
                            m_value = st.number_input(
                                f"{seg_name} 的 m",
                                min_value=0.01,
                                max_value=100.0,
                                value=float(existing_m),
                                step=0.1,
                                format="%.6g",
                                disabled=not use_fixed,
                                key=f"cross_m_value_{int(exp_id)}_{seg_name}",
                                help="固定后只锁定 m；η₀、η∞ 和 σc 仍会自动重新拟合。",
                            )
                        if use_fixed:
                            cross_m_overrides[str(seg_name)] = float(m_value)

                run_free_col, run_custom_col, dl_col = st.columns([1.0, 1.2, 1.0])
                with run_free_col:
                    if st.button("自由拟合全部 m", use_container_width=True, key=f"run_free_cross_{int(exp_id)}"):
                        try:
                            rdb.analyze_experiment(
                                db_path=db_path,
                                experiment_id=int(exp_id),
                                outdir=outdir,
                                cross_m_by_segment=None,
                            )
                            st.success("自由拟合完成 ✅")
                        except Exception as e:
                            st.exception(e)
                with run_custom_col:
                    if st.button("按当前 m 设置重新拟合", use_container_width=True, key=f"run_custom_cross_{int(exp_id)}"):
                        try:
                            rdb.analyze_experiment(
                                db_path=db_path,
                                experiment_id=int(exp_id),
                                outdir=outdir,
                                cross_m_by_segment=cross_m_overrides,
                            )
                            if cross_m_overrides:
                                fixed_text = ", ".join(f"{k}: m={v:g}" for k, v in cross_m_overrides.items())
                                st.success(f"重新拟合完成 ✅ 固定：{fixed_text}")
                            else:
                                st.success("没有勾选固定 m；已按 free m 重新拟合 ✅")
                        except Exception as e:
                            st.exception(e)
                with dl_col:
                    if outdir.exists():
            ...
            if outdir.exists():
                fig_paths = [
                    ...
                    outdir / "fig5_hb_fit_tau_vs_gammadot.png",
                    outdir / "fig6_model_comparison_eta_vs_tau.png",
                    outdir / "fig7_cross_partial_yield_map.png",
                    outdir / "fig8_torque_vs_angular_velocity.png",
                    outdir / "fig9_stress_vs_angular_velocity.png",
                    outdir / "fig10_viscosity_vs_angular_velocity.png",
                ]
...
with TAB_COMPARE:
    ...
    st.markdown(
        "- 选多个已经上传的实验一起对比\n"
        "- 选 all / up / down 用于误差计算\n"
        "- 曲线叠加图可以把 up 和 down 分开画\n"
        "- 新增 η vs τ：y 轴 viscosity，x 轴 shear stress\n"
        "- 每个实验固定一个颜色，legend 显示实验编号 / 样本名 / 几何，不显示创建时间\n"
        "- 不再画 fit / trend 直线，只显示实测曲线\n"
        "- y 轴可以选择 log 10^n 或 linear 自定义刻度"
    )
    ...
    if not db_path.exists():
        ...
        if exp_df.empty:
            ...
            if len(selected_labels) < 2:
                ...
                with c2:
                    plot_pair_label = st.selectbox(
                        "图类型 / 坐标变量",
                        options=list(PLOT_PAIR_OPTIONS.keys()),
                        index=0,
                        help="新增选项：η vs τ，即 y 轴为 viscosity，x 轴为 shear stress。",
                    )
                    x_choice, y_choice = PLOT_PAIR_OPTIONS[plot_pair_label]
                    st.caption(f"x = {X_LABELS.get(x_choice, x_choice)}；y = {Y_LABELS.get(y_choice, y_choice)}")
                with c3:
                    interp_mode = st.selectbox(
                        "插值方式（用于误差计算）",
                        options=["log-log（推荐）", "log-x linear-y", "linear-linear"],
                        index=0,
                        help="误差需要把不同实验插值到同一组 x 点上。",
                    )
                ...
                for eid in selected_ids:
                    ...
                    x = pd.to_numeric(dseg[x_choice], errors="coerce").to_numpy(dtype=float)
                ...
                st.markdown("---")
                st.markdown(f"#### 选择 x 轴区间：{X_LABELS.get(x_choice, x_choice)}")
                ...
                if auto_range and np.isfinite(overlap_min) and np.isfinite(overlap_max) and overlap_min < overlap_max:
                    x_min = overlap_min
                    x_max = overlap_max
                    st.info(f"共同区间：{x_min:.6g} ~ {x_max:.6g} ({X_LABELS.get(x_choice, x_choice)})")
                else:
                    if auto_range:
                    ...
                    with xr1:
                        x_min = st.number_input(
                            f"x_min — {X_LABELS.get(x_choice, x_choice)}",
                            value=float(overlap_min) if np.isfinite(overlap_min) else gmin,
                            min_value=0.0,
                        )
                    with xr2:
                        x_max = st.number_input(
                            f"x_max — {X_LABELS.get(x_choice, x_choice)}",
                ...
                st.markdown("---")
                st.markdown("#### 多实验叠加图设置")
                p1, p2, p3 = st.columns([1.25, 1.05, 1.0])
                with p1:
                    overlay_mode = st.selectbox(
                        "曲线图显示方式",
                        options=["up + down 同一张图", "只画 up", "只画 down", "all"],
                        index=0,
                        help="默认样式：同一实验同一颜色；up 用圆点，down 用方块；不画 fit 直线。",
                    )
                with p2:
                    y_axis_mode = st.selectbox(
                        "曲线图 y 轴刻度",
                        options=["log：10^n", "linear：自动", "linear：自定义刻度"],
                        index=0,
                        help="log 会显示 10 的幂；自定义刻度可以输入例如 1, 10, 15, 20。",
                    )
                with p3:
                ...
                custom_y_ticks_text = ""
                if y_axis_mode == "linear：自定义刻度":
                    custom_y_ticks_text = st.text_input(
                        "自定义 y 轴刻度（用逗号分隔）",
                        value="1, 10, 15, 20",
                        help="例如：1, 10, 15, 20；或 0, 5, 10, 15, 20。",
                    )
                show_markers = st.checkbox("曲线上显示原始数据点 marker", value=True)
                show_end_labels = st.checkbox(
                    "在曲线末端标注样本名（实验多时可能拥挤）",
                    value=False,
                )
                ...
                if run_btn and x_max > x_min:
                    with st.spinner("正在计算对比与误差..."):
                        ...
                        for eid in selected_ids:
                            df0 = rdb.measurements_df(db_path, int(eid))
                            x, y = _prepare_xy(df0, seg=seg_choice, x_col=x_choice, y_col=y_choice, x_min=float(x_min), x_max=float(x_max))
                        ...
                        if x_ref.size < 2:
                            ...
                            rows = []
                            aligned = pd.DataFrame({x_choice: xg, f"ref_{ref_id}": yref_g})
                            ...
                            st.markdown("### 多实验曲线叠加")
                            st.caption(
                                "同一张图同时显示 up/down：同一实验同一颜色，up 用圆点，down 用方块；这里只画实测曲线，不画 fit / trend 直线。"
                            )
                            fig, n_plotted = _plot_multi_experiment_overlay(
                                db_path=db_path,
                                ...
                                show_end_labels=show_end_labels,
                            )
                            if n_plotted == 0:
                                st.warning("当前 x 区间内没有可画的数据。")
                                plt.close(fig)
                            else:
                                st.pyplot(fig, use_container_width=True)
                                plt.close(fig)
                            ...
                            st.markdown("---")
                            st.markdown(f"### 误差分布（APE vs {X_LABELS.get(x_choice, x_choice)}）")
                            st.caption("这里显示相对 reference 的绝对百分比误差；只显示点，不用实线连接。")
                            ...
                            try:
                                ...
                                pass
                            ax2.set_xlabel(X_LABELS.get(x_choice, x_choice))
                            ax2.set_ylabel("Absolute percent error (%)")
                            ax2.set_title(f"APE vs {X_LABELS.get(x_choice, x_choice)} (reference = {ref_id})")
                            ax2.legend()
                            fig2.tight_layout()
