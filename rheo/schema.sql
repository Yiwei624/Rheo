...
CREATE TABLE IF NOT EXISTS fit_results (
    ...
    model                TEXT    NOT NULL, -- 'herschel_bulkley' | 'cross' | 'ellis'
    tau_y_Pa             REAL,             -- HB: yield stress τy
    K                    REAL,             -- HB: consistency index
    n                    REAL,             -- HB: flow index
    eta0_Pa_s            REAL,             -- Cross/Ellis: zero-shear viscosity η0
    eta_inf_Pa_s         REAL,             -- Cross: infinite-shear viscosity η∞; Ellis uses 0
    sigma_c_Pa           REAL,             -- Cross/Ellis: critical stress σc
    m                    REAL,             -- Cross/Ellis: shear-thinning index
    r_squared            REAL,
    n_points             INTEGER,
    method               TEXT,
    created_at           TEXT    NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS derived_metrics (
    ...
    tau1c_cross_up_Pa        REAL,   -- Cross-informed τ1,c = σc,Cross*(r2/r1)^2 (up)
    tau1c_cross_down_Pa      REAL,
    tau1c_cross_all_Pa       REAL,
    sigma_c_cross_up_Pa      REAL,   -- Cross critical stress σc used as apparent yield stress (up)
    sigma_c_cross_down_Pa    REAL,
    sigma_c_cross_all_Pa     REAL,
    cross_partial_fraction_up   REAL, -- fraction of points with τ1 < τ1,c,Cross
    cross_partial_fraction_down REAL,
    cross_partial_fraction_all  REAL,
    tauy_up_Pa               REAL,
    tauy_down_Pa             REAL,
    tauy_all_Pa              REAL,
    created_at               TEXT    NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS artifacts (
    id              INTEGER PRIMARY KEY AUTOINCREMENT,
    experiment_id    INTEGER NOT NULL,
    kind            TEXT NOT NULL,   -- 'fig1'...'fig7'|'summary_csv'|'overview_xlsx'
    path            TEXT NOT NULL,
    created_at      TEXT NOT NULL DEFAULT (datetime('now')),
    FOREIGN KEY (experiment_id) REFERENCES experiments(id) ON DELETE CASCADE
);
